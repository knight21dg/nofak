...
import 'package:nofak/data/repositories/system_repository.dart';
import 'package:nofak/utils/constant.dart';
import 'package:nofak/utils/hive_utils.dart';
import 'package:nofak/utils/network/network_availability.dart';
...
class FetchSystemSettingsCubit extends Cubit<FetchSystemSettingsState> {
  FetchSystemSettingsCubit() : super(
    HiveUtils.getSystemSettings() != null
        ? FetchSystemSettingsSuccess(settings: HiveUtils.getSystemSettings()!)
        : FetchSystemSettingsInitial()
  ) {
    final cached = HiveUtils.getSystemSettings();
    if (cached != null) {
      try {
        _updateConstants(cached);
      } catch (e, st) {
        log('Error loading cached system settings: $e\n$st');
      }
    }
  }
  ...
  Future<void> _fetchAndUpdateSettings() async {
    try {
      ...
      HiveUtils.storeSystemSettings(settings);
  ...
  void _updateConstants(Map settings) {
    ...
    // Location settings
    Constant.defaultLatitude = _getSetting(
      settings,
      SystemSetting.defaultLatitude,
    );
    Constant.defaultLongitude = _getSetting(
      settings,
    ...
    // Radius settings
    Constant.minRadius = double.tryParse(
          _getSetting(settings, SystemSetting.minRadius).toString(),
        ) ??
        0.0;
    Constant.maxRadius = double.tryParse(
          _getSetting(settings, SystemSetting.maxRadius).toString(),
        ) ??
        0.0;
    Constant.defaultLocation = Constant.defaultLocation.copyWith(
      radius: Constant.minRadius,
    );
    if (Constant.minRadius != HiveUtils.getLocationV2()?.radius) {
      final persistedLocation = HiveUtils.getLocationV2();
