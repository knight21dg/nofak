...
class MakeAnOfferItemCubit extends Cubit<MakeAnOfferItemState> {
  ...
  Future<void> makeAnOfferItem(
      {int? id, required String from, double? amount, int? technicianId}) async {
    emit(MakeAnOfferItemInProgress());

    await _itemRepository.makeAnOfferItem(id: id, amount: amount, technicianId: technicianId).then((value) {
