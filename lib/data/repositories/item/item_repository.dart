...
class ItemRepository {
  Future<ItemModel> createItem(
    ...
    try {
      ...
      parameters.addAll({
        ...
        Api.packageType: 'item_listing',
      });
      if (video != null) {
        parameters[Api.video] = video;
      }
      ...
      if (response['data'] == null || (response['data'] as List).isEmpty) {
        throw ApiException("Server returned success but no item data was found.");
      }
  ...
  Future<ItemModel> editItem(
    ...
    parameters[Api.packageType] = 'item_listing';
  ...
  Future<Map> makeAnOfferItem({int? id, double? amount, int? technicianId}) async {
    Map response = await Api.post(
      url: Api.itemOfferApi,
      parameter: {
        if (id != null) Api.itemId: id,
  ...
  Future<Map> deductCallCredits(int id) async {
    Map response = await Api.post(
      url: Api.deductCallCreditsApi,
