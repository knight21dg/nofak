...
import 'package:nofak/data/model/location/location_node.dart';
import 'package:nofak/utils/api.dart';
import 'package:nofak/utils/constant.dart';
import 'package:nofak/utils/json_helper.dart';
...
class LocationRepository {
  ...
  Future<LeafLocation> getLocationFromPlaceId({required String placeId}) async {
    try {
      final response = await Api.get(
        url: Api.getLocationApi,
        queryParameters: {'place_id': placeId, Api.lang: 'EN'},
      ...
      return _extractLeafLocation(results.first);
    } on Exception catch (e, stack) {
  ...
  LeafLocation _extractLeafLocation(Map<String, dynamic> json) {
    ...
    // Combine parts for a precise "Area" with fallbacks
    final areaParts = [
      if (premise != null) premise,
      if (streetNumber != null) streetNumber,
      if (route != null) route,
      if (sublocality != null) sublocality,
      if (neighborhood != null && sublocality == null) neighborhood,
    ];

    if (areaParts.isNotEmpty) {
      leafLocationJson['area'] = areaParts.join(', ');
    } else {
      // Final safety fallback: use the formatted address if area is still empty
      leafLocationJson['area'] = json['formatted_address']?.toString().split(',').first;
    }
