...
import 'package:nofak/ui/screens/widgets/skip_button_widget.dart';
      ...
      if (state is MFail) {
        ...
        if (isOtpSent && (otp == null || otp!.trim().isEmpty)) {
          ...
          if (mounted) {
            HelperUtils.showSnackBarMessage(
              context,
              ErrorFilter.getTranslatedFirebaseAuthException(
                context,
                error: state.error,
              ),
            );
              ...
              listener: (context, state) {
                if (state is LoginSuccess) {
                  ...
                  if (state.isProfileCompleted) {
                    ...
                    final accountType = state.apiResponse['account_type'];
                    if (accountType == null || accountType.toString().isEmpty) {
                      ...
                      if (location != null && !location.isEmpty) {
                        HelperUtils.killPreviousPages(context, Routes.main, {
                          "from": widget.isNewUser ? "signup" : "login",
                        });
                      } else {
                        Navigator.of(context).pushNamedAndRemoveUntil(
              ...
              child: BlocConsumer<AuthenticationCubit, AuthenticationState>(
                listener: (context, state) {
                  if (state is AuthenticationSuccess) {
                    LoadingWidgets.hideLoader(context);
                    String? referralCode = state.payload.referralCode;
                    ...
                    } else if (state.type == AuthenticationType.apple) {
                      context.read<LoginCubit>().login(
                            type: state.type.name,
                            phoneNumber: state.credential.user!.phoneNumber,
                            firebaseUserId: state.credential.user!.uid,
