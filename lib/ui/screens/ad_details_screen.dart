...
import 'package:nofak/data/cubits/item/change_my_items_status_cubit.dart';
import 'package:nofak/data/cubits/item/create_featured_ad_cubit.dart';
import 'package:nofak/data/cubits/item/delete_item_cubit.dart';
import 'package:nofak/data/cubits/item/fetch_item_cubit.dart';
import 'package:nofak/data/cubits/item/fetch_my_item_cubit.dart';
import 'package:nofak/data/cubits/item/item_total_click_cubit.dart';
import 'package:nofak/data/cubits/item/job_application/fetch_job_application_cubit.dart';
import 'package:nofak/data/cubits/item/related_item_cubit.dart';
import 'package:nofak/data/cubits/location/leaf_location_cubit.dart';
import 'package:nofak/data/cubits/renew_item_cubit.dart';
import 'package:nofak/data/cubits/report/fetch_item_report_reason_list.dart';
import 'package:nofak/data/cubits/report/item_report_cubit.dart';
import 'package:nofak/data/cubits/report/update_report_items_list_cubit.dart';
import 'package:nofak/data/cubits/safety_tips_cubit.dart';
import 'package:nofak/data/cubits/seller/fetch_seller_ratings_cubit.dart';
import 'package:nofak/data/cubits/subscription/fetch_user_credit_limit_cubit.dart';
...
import 'package:nofak/data/model/chat/chat_user_model.dart';
import 'package:nofak/data/model/item/item_model.dart';
import 'package:nofak/data/model/item/job_application.dart'
    show JobApplication;
import 'package:nofak/data/model/location/leaf_location.dart';
import 'package:nofak/data/model/report_item/reason_model.dart';
import 'package:nofak/data/model/safety_tips_model.dart';
import 'package:nofak/data/model/user/user_model.dart';
import 'package:nofak/ui/screens/ad_banner_screen.dart';
import 'package:nofak/ui/screens/chat/chat_screen.dart';
import 'package:nofak/ui/screens/google_map_screen.dart';
...
import 'package:nofak/utils/validator.dart';
...
class AdDetailsScreenState extends CloudState<AdDetailsScreen> {
  ...
  void initVariables(ItemModel itemModel) {
    ...
    if (isAddedByMe) {
      // Package fetch removed as we moved to credit-only model
    } else {
  ...
  void setItemClick() {
    if (!isAddedByMe) {
      if (HiveUtils.isUserAuthenticated()) {
        ...
        int listingViewDeduction = int.tryParse(context.read<FetchSystemSettingsCubit>().getRawSettings()['listing_view_deduction']?.toString() ?? '1') ?? 1;

        if ((userAuth.credits ?? 0) < listingViewDeduction) {
      ...
      child: BlocConsumer<FetchItemCubit, FetchItemState>(
        ...
        builder: (context, state) {
          ...
          return MultiBlocListener(
            listeners: [
              BlocListener<ItemTotalClickCubit, ItemTotalClickState>(
                listener: (context, clickState) {
                  if (clickState is ItemTotalClickSuccess) {
              ...
              BlocListener<RenewItemCubit, RenewItemState>(
                listener: (context, changeState) {
                  ...
                  if (changeState is RenewItemInSuccess) {
                    ...
                    LoadingWidgets.hideLoader(context);
                    UiUtils.showError(context, changeState.error);
                  }
                },
              ),
            ],
  ...
  Widget buildRelatedListWidget(FetchRelatedItemsSuccess state) {
    // Filter out the current item to avoid empty spaces in the grid
    final filteredItems =
  ...
  Widget createFeaturesAds() {
    if (model.status == Constant.statusActive ||
      ...
      return MultiBlocProvider(
        providers: [
          BlocProvider(create: (context) => CreateFeaturedAdCubit()),
          BlocProvider(create: (context) => FetchUserCreditLimitCubit()),
        ],
        child: Builder(
          builder: (context) {
            return BlocListener<CreateFeaturedAdCubit, CreateFeaturedAdState>(
              ...
              child:
                  BlocListener<
                    FetchUserCreditLimitCubit,
                    FetchUserCreditLimitState
                  >(
                    listener: (context, state) async {
                      if (state is FetchUserCreditLimitFailure) {
                        UiUtils.insufficientCreditsDialog(context);
                      }
                      if (state is FetchUserCreditLimitInSuccess) {
                        await UiUtils.showBlurredDialoge(
                          context,
                          dialoge: BlurredDialogBox(
                            ...
                            onAccept: () => Future.value().then((_) {
                              if (context
                                      .read<FetchUserCreditLimitCubit>()
                                      .state
                                  is FetchUserCreditLimitInProgress) {
                                return;
                              }
                              Future.delayed(Duration.zero, () {
                                context
  ...
  Widget customFields() {
    // Use allTranslatedCustomFields for displaying all custom fields
    ...
    // Group fields by id and language
    ...
    // For each id, pick the field for the current language if it exists, otherwise pick any
    ...
    return BlocListener<ChangeMyItemStatusCubit, ChangeMyItemStatusState>(
      listener: (context, changeState) {
        if (changeState is ChangeMyItemStatusSuccess) {
          ...
          UiUtils.showError(context, changeState.errorMessage);
        }
      },
      child: Expanded(
        child: _buildButton(
              ...
              child: _buildButton(
                ...
                () async {
                  var accept = await UiUtils.showBlurredDialoge(
                    context,
                    dialoge: BlurredDialogBox(
                      title: "renew".translate(context),
                      content: CustomText("renewConfirmMsg".translate(context)),
              ...
              return BlocListener<MakeAnOfferItemCubit, MakeAnOfferItemState>(
                listener: (context, state) {
                  if (state is MakeAnOfferItemSuccess) {
                    ...
                    // Refresh user profile and credit balance after initiating chat/offer (deducting credits)
                    context.read<UserProfileCubit>().getUserProfile().then((_) {
                      context.read<UserDetailsCubit>().fill(HiveUtils.getUserDetails());
                    });
                ...
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    if ((model.contact ?? model.user?.mobile ?? '').isNotEmpty)
                      Padding(
                            ...
                            onPressed: () async {
                              UiUtils.checkUser(
                                onNotGuest: () async {
                                  ...
                                  bool isOwner = model.user?.id != null && model.user?.id == HiveUtils.getUserDetails().id;

                                  if (isOwner) {
                                    // Connection already exists, free call
                                    final String rawPhone = model.contact ?? model.user?.mobile ?? "";
                                    final String phone = formatPhoneNumber(rawPhone, Constant.defaultCountryCode);
                                    ...
                                    if (await canLaunchUrl(launchUri)) {
                                      ...
                                      HelperUtils.showSnackBarMessage(
                                        context,
                                        "Could not launch dialer".translate(context),
                                      );
                                    }
                                  } else {
                                    // Credit Check & Deduction on every call
                                    int callDeduction = int.tryParse(context.read<FetchSystemSettingsCubit>().getRawSettings()['call_deduction']?.toString() ?? '5') ?? 5;
                                    UserModel userAuth = HiveUtils.getUserDetails();

                                    if ((userAuth.credits ?? 0) < callDeduction) {
                                      UiUtils.showBlurredDialoge(
                                        context,
                                        dialoge: BlurredDialogBox(
                                          title: "Recharge Required".translate(context),
                                          content: CustomText(
                                            "Initiating this call requires $callDeduction NOFAK Credits. Your current balance is ${userAuth.credits}."
                                                .translate(context),
                                          ),
                                      ...
                                      UiUtils.showBlurredDialoge(
                                        context,
                                        dialoge: BlurredDialogBox(
                                          title: "Confirm Call".translate(context),
                                          content: CustomText(
                                            "Calling this seller requires $callDeduction NOFAK Credits. Your current balance is ${userAuth.credits}. Do you want to proceed?"
                                                .translate(context),
                                          ),
                                          ...
                                          onAccept: () async {
                                            ...
                                            try {
                                              LoadingWidgets.showLoader(context);
                                              var response = await ItemRepository().deductCallCredits(model.id!);
                                              LoadingWidgets.hideLoader(context);

                                              // Update local credits in Hive and UserDetailsCubit
                                              int updatedCredits = response['data'] != null && response['data']['credits'] != null
                                                  ? (response['data']['credits'] is int
                                                      ? response['data']['credits']
                                                      : int.parse(response['data']['credits'].toString()))
                                                  : ((userAuth.credits ?? 0) - callDeduction);
                                              
                                              userAuth.credits = updatedCredits;
                                              HiveUtils.setUserData(userAuth.toJson());
                                              context.read<UserDetailsCubit>().fill(userAuth);

                                              // Fetch chat buyer list to set chatedUser so subsequent calls/chats are free
                        ...
                        Expanded(
                          child: _buildButton(
                            ...
                            () {
                              UiUtils.checkUser(
                                onNotGuest: () {
                                  DateTime? lastShown = HiveUtils.getLastSafetyTipsShowDate();
                                  DateTime now = DateTime.now();
                                  bool isNewDay = lastShown == null ||
                                      lastShown.day != now.day ||
                                      lastShown.month != now.month ||
                                      lastShown.year != now.year;

                                  if (isNewDay) {
                                    safetyTipsBottomSheet(onContinue: () {
                            ...
                            onNotGuest: () {
                              void proceedToChat() {
                                if (chatedUser != null) {
                                  ...
                                  // Wallet Economy: Credit check (Spec 4.1 & 4.5)
                                  UserModel userAuth = HiveUtils.getUserDetails();
                                  int chatDeduction = int.tryParse(context.read<FetchSystemSettingsCubit>().getRawSettings()['chat_deduction']?.toString() ?? '5') ?? 5;
                              ...
                              if (isNewDay) {
                                safetyTipsBottomSheet(onContinue: proceedToChat);
                              } else {
                                proceedToChat();
                              }
                            },
  ...
  void safetyTipsBottomSheet({VoidCallback? onContinue}) {
          ...
          child: ListView(
            ...
            children: [
              ...
              Padding(
                ...
                child: _buildButton(
                  "continueLbl".translate(context),
                  () {
                    HiveUtils.setLastSafetyTipsShowDate(DateTime.now());
                    Navigator.pop(context);
                    if (onContinue != null) {
                      onContinue();
                    } else {
  ...
  Widget setSellerDetails() {
    bool isTechnician = model.category?.slug == 'technician-marketplace' ||
        model.category?.name?.toLowerCase().contains('technician') == true ||
        model.category?.name?.toLowerCase().contains('professional') == true;

    // Determine expertise icon
    IconData expertiseIcon = Icons.handyman_rounded;
    if (model.category?.name?.toLowerCase().contains("ac") ?? false) {
      expertiseIcon = Icons.ac_unit_rounded;
    } else if (model.category?.name?.toLowerCase().contains("plumbing") ?? false) {
      expertiseIcon = Icons.plumbing_rounded;
    } else if (model.category?.name?.toLowerCase().contains("electrical") ?? false) {
      expertiseIcon = Icons.electric_bolt_rounded;
    } else if (model.category?.name?.toLowerCase().contains("cars") ?? false) {
    ...
    return Padding(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: isTechnician
              ? Border.all(color: context.color.territoryColor.withValues(alpha: 0.1))
              : null,
        ),
        child: Row(
          children: [
                  ...
                  Container(
                    ...
                    child: ClipRRect(
                      ...
                      child: model.user!.profile != null && model.user!.profile != ""
                          ? UiUtils.getImage(model.user!.profile!, fit: BoxFit.cover)
                          : UiUtils.getSvg(
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                      ...
                      Padding(
                        ...
                        child: Row(
                          children: [
                            ...
                            CustomText(
                              "Verified Specialist".translate(context),
                              fontSize: context.font.smaller,
                              color: context.color.territoryColor,
                              fontWeight: FontWeight.bold,
                            ),
                            if (model.user?.isAddressVerified == 1) ...[
                              const SizedBox(width: 8),
                              Container(
                                ...
                                child: Row(
                                  mainAxisSize: MainAxisSize.min,
                                  children: [
                                    Icon(Icons.location_on_rounded, size: 10, color: context.color.forthColor),
                                    const SizedBox(width: 2),
                                    CustomText(
                                      "Verified Location".translate(context),
                                      fontSize: 9,
                                      color: context.color.forthColor,
                                      fontWeight: FontWeight.bold,
                                    ),
                    ...
                    if (model.user?.accountType != null && model.user!.accountType!.isNotEmpty) ...[
                      const SizedBox(height: 4),
                      Align(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            color: (model.user!.accountType == 'individual'
                                    ? context.color.territoryColor
                          ...
                          child: CustomText(
                            model.user!.accountType == 'individual'
                      ...
                      Padding(
                        ...
                        child: Row(
                          children: [
                            ...
                            const SizedBox(width: 4),
                            CustomText(
                              context.watch<FetchSellerRatingsCubit>().sellerData()!.averageRating!.toStringAsFixed(1),
                              fontSize: context.font.small,
                              fontWeight: FontWeight.bold,
                            ),
                            const SizedBox(width: 4),
