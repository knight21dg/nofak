...
class UserProfileScreenState extends State<UserProfileScreen> {
  ...
  void initState() {
    ...
    String rawPhone = HiveUtils.getUserDetails().mobile ?? "";
    String cCodeRaw = HiveUtils.getCountryCode() ?? Constant.defaultCountryCode;
    String cleanCCode = cCodeRaw.replaceAll('+', '');

    if (cleanCCode.isNotEmpty && rawPhone.startsWith(cleanCCode)) {
      phoneController.text = rawPhone.substring(cleanCCode.length);
    } else {
      phoneController.text = rawPhone;
    }

    if (cCodeRaw.isNotEmpty && !cCodeRaw.startsWith('+')) {
      countryCode = "+$cCodeRaw";
    } else if (cCodeRaw.isNotEmpty) {
      countryCode = cCodeRaw;
    }
    ...
    return Column(
      ...
      children: [
        ...
        CustomTextFormField(
          ...
          isReadOnly: HiveUtils.getUserDetails().type == AuthenticationType.phone.name,
      ...
      if (isFromLogin) {
        Future.delayed(Duration.zero, () {
          if (widget.popToCurrent ?? false) {
            ...
            if (user.accountType == null || user.accountType!.isEmpty) {
              Navigator.of(context).pushNamedAndRemoveUntil(
                Routes.chooseAccountType,
                (route) => false,
                arguments: {"isNewUser": widget.isNewUser},
              );
            } else {
              final location = HiveUtils.getLocationV2();
              if (location != null && (location.city != null || location.area != null)) {
