...
import 'package:nofak/data/model/item/item_filter_model.dart';
...
import 'package:nofak/ui/screens/home/widgets/item_horizontal_card.dart';
...
class ItemsListState extends State<ItemsList> {
  ...
  void initState() {
    super.initState();
    // Reset cubit to prevent showing stale data from previous categories
    context.read<FetchItemFromCategoryCubit>().setInitialState();
  ...
  Widget mainChildren(List<ItemModel> items) {
    ...
    // Calculate total slots (items + ads)
    List<dynamic> listItems = [];
    for (int i = 0; i < items.length; i++) {
      listItems.add(items[i]);
      if ((i + 1) % gridCount == 0 && i != items.length - 1) {
        listItems.add("ad");
      }
    }
    ...
    return RefreshIndicator(
      ...
      child: isList 
        ? ListView.builder(
            ...
            padding: const EdgeInsetsDirectional.only(bottom: 30, start: 15, end: 15, top: 10),
        ...
        : GridView.builder(
            ...
            itemBuilder: (context, index) {
               var item = listItems[index];
               if (item == "ad") {
                 // For grid, ads might need to span full width
                 // Since this is a standard GridView, we'll just show it in a cell
                 // or use a CustomScrollView for better ad spanning.
                 return const NativeAdWidget(type: TemplateType.medium);
               }
               return GestureDetector(
                 onTap: () => _navigateToDetails(context, item),
