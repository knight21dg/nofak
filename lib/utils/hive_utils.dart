...
class HiveUtils {
  ...
  static bool isTutorialShown() {
    return Hive.box(HiveKeys.authBox).get("isTutorialShown") ?? false;
  }

  static Future<void> setTutorialShown() {
    return Hive.box(HiveKeys.authBox).put("isTutorialShown", true);
  }
  ...
  static Future<void> setLastSafetyTipsShowDate(DateTime date) async {
    await Hive.box(HiveKeys.userDetailsBox)
        .put("lastSafetyTipsShowDate", date.toIso8601String());
  }

  static DateTime? getLastSafetyTipsShowDate() {
    String? dateStr = Hive.box(HiveKeys.userDetailsBox).get("lastSafetyTipsShowDate");
    return dateStr != null ? DateTime.parse(dateStr) : null;
  }

  static void saveAddItemDraft(Map draft) {
    Hive.box(HiveKeys.draftBox).put(HiveKeys.addItemDraftKey, draft);
  }

  static Map? getAddItemDraft() {
    return Hive.box(HiveKeys.draftBox).get(HiveKeys.addItemDraftKey) as Map?;
  }

  static void clearAddItemDraft() {
    Hive.box(HiveKeys.draftBox).delete(HiveKeys.addItemDraftKey);
  }

  static void storeSystemSettings(Map settings) {
    Hive.box(HiveKeys.userDetailsBox).put("systemSettings", settings);
  }

  static Map? getSystemSettings() {
    final data = Hive.box(HiveKeys.userDetailsBox).get("systemSettings");
