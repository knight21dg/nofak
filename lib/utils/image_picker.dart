...
class PickImage {
  ...
  static List<File>? recoveredFiles;
  ...
  Future<void> pick(
    ...
    try {
      // Clear image cache to free up memory before launching the memory-intensive camera
  ...
  void injectFiles(List<File> files) {
    if (files.isNotEmpty) {
      pickedFile = files[0];
    }
    _sink.add({
  ...
  Future<void> checkLostData() async {
    try {
      ...
      if (response.file != null) {
        ...
        List<File> files = [];
        for (var xFile in response.files!) {
