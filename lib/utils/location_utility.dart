...
import 'package:nofak/data/repositories/location/location_repository.dart';
...
final class LocationUtility {
  ...
  Future<LeafLocation?> getLocation(BuildContext context) async {
    ...
    if (permissionGiven && locationServiceEnabled) {
      await _getLiveLocation(forceFresh: true);
      return location;
    } else {
  ...
  Future<void> _getLiveLocation({bool forceFresh = false}) async {
    ...
    if (position == null) {
      ...
      // forceFresh=true skips the distance check so we always get a real GPS fix
      bool shouldFetch = forceFresh || true;
      if (!forceFresh && (location?.hasCoordinates ?? false)) {
